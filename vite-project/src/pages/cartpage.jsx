...
export default function CartPage() {
	...
	return (
		<div className="w-full flex flex-col items-center p-[20px]">
			{cart.map((item,index) => {
				return (
					<div key={index} className="w-full h-[170px] lg:w-[50%] lg:h-[150px] rounded-xl pt-[20px] overflow-hidden relative shadow-2xl my-1 flex justify-between">
						<h4 className="lg:hidden w-full overflow-hidden h-[20px] absolute left-0 top-[0px]">
							{item.name}
						</h4>
			...
			<div className=" w-full lg:w-[50%] h-[150px] rounded-xl overflow-hidden shadow-2xl my-1 flex justify-between items-center">
				<Link
					to="/checkout"
					className="self-center ml-4 px-6 py-3 rounded bg-white text-black hover:bg-accent/90 transition"
					state={cart}
				>
					Checkout
				</Link>
